...
void receiveMessage(int fd) {
	...
	char *from = &buf[0];
	int fromLen = strlen(from) + 1;
	char *msg = &from[fromLen];

	if(strcmp("OK", from) || strcmp("ERROR", from)) {
	//Print "sender: msg" to stdout
		if(from[0] != '\n' && from[0] != '\0')
			dprintf(1, "<%s> %s", from, msg);
	}
}
